	...
	FR.update = function() {
		...
		Ext.each(window.parent.FR.sendingByEmail.items, function(item) {
			...
			if (item.isFolder) {
				...
				html += '<div style="float:left;max-width:220px;overflow:hidden;text-overflow: ellipsis;margin-top:1px;font-size:11px"><img src="'+URLRoot+'/images/fico/'+item.icon+'" align="top" style="margin-right:3px;" width="16" height="16" border="0"> '+item.filename+'</div>';
	...
	FR.UI.webLinksForm = new Ext.FormPanel({
		...
		items: [
			...
			},
			{
				...
				xtype: 'checkbox', name: 'wl_force_save', id: 'wl_force_save', value: '1', hideLabel: true
			},
			{
				boxLabel: FR.T('Receive download e-mail notifications.'),
